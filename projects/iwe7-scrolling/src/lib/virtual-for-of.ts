...
import { ListRange } from "./collection-viewer";
import {
  Directive,
  DoCheck,
  EmbeddedViewRef,
...
export class CdkVirtualForOf<T>
  implements CollectionViewer, DoCheck, OnDestroy {
  ...
  dataStream: Observable<T[]> = this._dataSourceChanges.pipe(
  ...
  measureRangeSize(
    ...
    while (i--) {
      ...
      while (j--) {
        totalSize += getSize(
          orientation,
          (view!.rootNodes[j] as Element).getBoundingClientRect()
        );
      }
    }
    ...
    changes.forEachOperation(
      (
        ...
        if (record.previousIndex == null) {
          ...
          // Item removed.
          this._cacheView(this._viewContainerRef.detach(
