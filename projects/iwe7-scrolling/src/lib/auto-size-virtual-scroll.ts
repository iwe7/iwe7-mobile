    ...
    if (underscan > 0) {
      ...
      if (scrollMagnitude >= viewport.getViewportSize()) {
        ...
        const addItems = Math.max(
          0,
          Math.ceil(
            (underscan - this._minBufferPx + this._addBufferPx) /
...
export class CdkAutoSizeVirtualScroll implements OnChanges {
   ...
   */
  @Input()
  get addBufferPx(): number {
    return this._addBufferPx;
  }
  set addBufferPx(value: number) {
    this._addBufferPx = coerceNumberProperty(value);
  }
  _addBufferPx = 200;
