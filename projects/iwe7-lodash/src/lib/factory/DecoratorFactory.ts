  ...
  createInstanceDecorator(config: DecoratorConfig): GenericDecorator {
    ...
    return (...args: any[]): LodashDecorator => {
      ...
      const decorator = (
        ...
        const isProperty =
          isFirstInstance && !isGetter && !isSetter && !isMethod;

        chainData.properties.push(name as any);
        chainData.fns.push(
        ...
        InstanceChainMap.set([target, name], chainData as any);

        if (!isFirstInstance) {
          return descriptor;
        }

        (<any>chainData).isSetter = isSetter;
        (<any>chainData).isGetter = isGetter;
        (<any>chainData).isMethod = isMethod;
        (<any>chainData).isProperty = isProperty;
        ...
        const applyChain = (
          ...
          return chainData.fns.reduce(
            (result: Function, next: Function) =>
              next(result, instance, context),
            fn
          );
        };
